...
class TicketController extends Controller
{
    ...
    public function post_newticket(CreateTicketRequest $request, CountryCode $code, $api = false)
    {
        try {
            ...
            $form_data = $request->except('name', 'phone', 'email', 'subject', 'body', 'helptopic', '_wysihtml5_mode', '_token', 'mobile', 'code', 'priority', 'attachment', 'first_name', 'last_name', 'sla', 'duedate', 'assignto', 'type_id', 'helptopic_par', 'helptopic_sub');
            $auto_response = 0;
            $status = 1;
        ...
        try {
            ...
            if ($system_reply == true) {
                if ($tickets->assigned_to == 0) {
                    ...
                    $tickets->sendToTelegram(Auth::user(), 'message', $thread2->body);
                }
                if ($tickets->status > 1) {
            ...
            $thread->save();
            $tickets->sendToTelegram(Auth::user(), 'message', $thread->body);
...
//                Event fire for new ticket['']
    ...
    public function createTicket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $type_id)
    {
        ...
        if ($this->ticketThread($subject, $body, $id, $user_id, false) == true) {
     ...
     * @return bool
     */
    public function ticketThread($subject, $body, $id, $user_id, $doSendTG=true)
    {
        ...
        if ($thread->save()) {
            $ticket = Tickets::query()->find($id);
            if ($doSendTG) $ticket->sendToTelegram(Auth::user(), 'message', $thread->title.' -- '.$thread->body);
    ...
    public function close($id, Tickets $ticket)
    {
        ...
        $ticket_status->sendToTelegram(Auth::user(), 'message', $thread->body);
...
//        $ticket_status = $ticket->where('id', '=', $id)->first();
    ...
    public function assign($tid)
    {
        ...
        foreach ($ticket_array as $id) {
            ...
            if ($assign_to[0] == 'team') {
                ...
                $ticket->sendToTelegram(Auth::user(), 'message', $thread->body);
    ...
    public function surrender($id)
    {
        ...
        $ticket->sendToTelegram(Auth::user(), 'message', $NewThread->body);
